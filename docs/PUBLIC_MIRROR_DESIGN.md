...
Only **analysis data** crosses the boundary. No raw combat-log lines, no silver
event detail, no bronze byte offsets.

**2026-06-29 correction:** the useful local detail page was never fully promoted
into gold. Frontend-visible encounter analysis needs gold-backed read models
before it can be mirrored. The four-table failure-fact payload below remains a
working plumbing contract, but it is not the final public product contract.

**2026-07 update:** the active replacement plan is file-based. Do not extend the
Phoenix ingest/database copy as the product path; use Cloudflare-hosted JSON
artifacts generated by the medallion build.
...
| v1 view scope | **Provisional failure-fact preview only.** The finished product scope is the gold-backed encounter detail contract in [`PUBLIC_MIRROR_GOLD_DETAIL_PLAN.md`](PUBLIC_MIRROR_GOLD_DETAIL_PLAN.md). |
...
Mirrored tables in the current implemented plumbing contract: `gold.dim_encounter`, `gold.dim_player`, `gold.dim_mechanic_criterion`, `gold.fact_failure`.

These four tables are **not** the final public data surface. They can support a
failure-fact preview only. The real public mirror needs a broader gold encounter
detail contract that includes roster, deaths, damage summaries, debuffs,
interrupt/cast summaries where defensible, and failure facts.
...
- **Per-encounter detail in the current plumbing** — a gold-only slice of failure facts: which players failed which criteria this pull, with counts and total damage attached to those facts. This is not the real encounter detail page.
- **Per-encounter detail in the corrected product** — see [`PUBLIC_MIRROR_GOLD_DETAIL_PLAN.md`](PUBLIC_MIRROR_GOLD_DETAIL_PLAN.md). The rich detail page should be rebuilt on public-safe gold read models instead of querying silver directly from the frontend.
...
## Implemented Plumbing Phases

Tracked on Linear: project [Public Gold Mirror](https://linear.app/we-go-next/project/public-gold-mirror-55ce8feabbb8) (workspace `we-go-next`, team `WE`), issues **WE-5 … WE-12**.

These phases built the current deploy/ingest/upload plumbing and provisional
failure-fact preview. They are not the active product plan for full encounter
analysis mirroring.
...
- ~~Per-encounter detail is gold-only~~ — **false**; `EncounterDetail` reads 8 silver tables. The WE-12 implementation replaced it with a provisional gold-only failure-breakdown read model. The corrected product direction is now a real gold-backed encounter detail contract; see `PUBLIC_MIRROR_GOLD_DETAIL_PLAN.md`.
